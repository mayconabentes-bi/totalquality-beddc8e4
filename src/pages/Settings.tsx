...
const Settings = () => {
  ...
  const generateNextClientCode = async (): Promise<string> => {
    try {
      // Fetch all existing companies to find the highest client code number
      const { data, error } = await supabase
        .from("companies")
        .select("client_code")
      ...
      if (error) {
        ...
        return "TQ-101"; // Default starting code
      }

      if (!data || data.length === 0) {
        return "TQ-101"; // First client code
      }

      // Extract the number from the last code (e.g., "TQ-101" -> 101)
      const lastCode = data[0].client_code;
      const match = lastCode?.match(/TQ-(\d+)/);
      
      if (match && match[1]) {
        const lastNumber = parseInt(match[1], 10);
        const nextNumber = lastNumber + 1;
        return `TQ-${nextNumber}`;
      }

      return "TQ-101"; // Fallback
    } catch (error) {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          client_code: newCompanyForm.client_code || null,
